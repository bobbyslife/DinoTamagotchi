...
# Create ultra-simple installer package for Dino Tamagotchi

echo "🦕 Creating simple Dino Tamagotchi package..."

# Create package directory
PKG_DIR="Dino-Tamagotchi-macOS"
rm -rf "$PKG_DIR"
mkdir -p "$PKG_DIR"

# Copy files
cp supabase_dino.py "$PKG_DIR/"
cp requirements.txt "$PKG_DIR/"
cp DinoTamagotchi.icns "$PKG_DIR/" 2>/dev/null || echo "No icon"

# Create a super simple double-clickable installer
cat > "$PKG_DIR/🦕 Install Dino Tamagotchi.command" << 'EOF'
#!/bin/bash
...
chmod +x "$PKG_DIR/🦕 Install Dino Tamagotchi.command"
...
# Create ZIP
zip -r "Dino-Tamagotchi-macOS.zip" "$PKG_DIR"

echo ""
echo "✅ Package created: Dino-Tamagotchi-macOS.zip"
echo "📦 Size: $(du -h Dino-Tamagotchi-macOS.zip | cut -f1)"
echo ""
echo "🎯 Users now just:"
echo "1. Download and unzip"
echo "2. Double-click '🦕 Install Dino Tamagotchi.command'"
